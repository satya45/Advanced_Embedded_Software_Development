 ...
 * Author @Satya Mehta
 * Server code which sends messages to client and receives.
...
void handler(int signo, siginfo_t *info, void *extra) 
{
    ...
    fptr = fopen("q_serverr_log.txt", "a");
...
int main()
{
	...
	mq = mq_open(QUEUE_N, O_RDWR, 0664, &attr);
	if(mq == -1)
	{
        perror("mq_open");
    }
	for(int i=0; i<10; i++)
    {
	    ...
	    timevalue += timestamp.tv_nsec/1000000; 
		fptr = fopen("q_serverr_log.txt", "a");
        fprintf(fptr,"TimeStamp: - %ld and message sent from server to client %s,  length %d, LED status %d\n", timevalue, x.string, x.length, x.led);
        fclose(fptr);
		sleep(1);
	...
	timevalue += timestamp.tv_nsec/1000000;
	fptr = fopen("q_serverr_log.txt", "a");
    fprintf(fptr,"TimeStamp: - %ld and message received from client %s,  length %d, LED status %d\n", timevalue, y.string, y.length, y.led);
    fclose(fptr);
	}	
	rc = mq_close(mq);
	//if(rc == -1)
	rc = mq_unlink(QUEUE_N);	
	return 0;
}